...
from omni.isaac.lab_assets.humanoid import HUMANOID_CFG
...
class MultiObjectSceneCfg(InteractiveSceneCfg):
    ...
    # articulation
    robot1: ArticulationCfg = ArticulationCfg(
    ...
    robot2: ArticulationCfg = HUMANOID_CFG.replace(prim_path="{ENV_REGEX_NS}/Humanoid")
    robot3: ArticulationCfg = HUMANOID_CFG.replace(prim_path="{ENV_REGEX_NS}/Humanoid2")
...
def run_simulator(sim: SimulationContext, scene: InteractiveScene):
    ...
    robot1: Articulation = scene["robot1"]
    robot2: Articulation = scene["robot2"]
    robot3: Articulation = scene["robot3"]
    ...
    while simulation_app.is_running():
        ...
        if count % 250 == 0:
            ...
            # -- reset the humanoid 1
            root_state = robot2.data.default_root_state.clone()
            root_state[:, :3] += scene.env_origins
            robot2.write_root_pose_to_sim(root_state[:, :7])
            robot2.write_root_velocity_to_sim(root_state[:, 7:])
            # -- joint state
            joint_pos, joint_vel = robot2.data.default_joint_pos.clone(), robot2.data.default_joint_vel.clone()
            robot2.write_joint_state_to_sim(joint_pos, joint_vel)
            # -- reset the humanoid 2
            root_state = robot3.data.default_root_state.clone()
            root_state[:, :3] += scene.env_origins
            robot3.write_root_pose_to_sim(root_state[:, :7])
            robot3.write_root_velocity_to_sim(root_state[:, 7:])
            # -- joint state
            joint_pos, joint_vel = robot3.data.default_joint_pos.clone(), robot3.data.default_joint_vel.clone()
            robot3.write_joint_state_to_sim(joint_pos, joint_vel)
        ...
        robot1.set_joint_position_target(robot1.data.default_joint_pos)
        # Write data to sim
