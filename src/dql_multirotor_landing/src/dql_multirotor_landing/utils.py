...
def create_log_dir_path(parent_folder_path: str, tb_log_name: str):
    ...
    log_dir_path = os.path.join(
        parent_folder_path, tb_log_name + "_" + str(subfolder_index + 1)
    )
    print("Data logging directory: ", log_dir_path)
    return log_dir_path


def detect_contact_in_msg(msg: ContactsState):
    """Function returns the value true if a contact has been detected by a contact sensor that publishes on the specified topic."""
    contact_detected = False
    if msg.states:
        contact_detected = True
    print(contact_detected)
