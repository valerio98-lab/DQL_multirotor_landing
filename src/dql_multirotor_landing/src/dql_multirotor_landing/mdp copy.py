...
# During training, the yaw controller ensures ψ_rel = 0
class Mdp:
    ...
    def discrete_state(
        ...
        if self.direction == "x":
            ...
            continuous_position = np.clip(
                current_continuous_observation.observation.rel_p_y / self.p_max, -1, 1
            )
            continuous_velocity = np.clip(
                current_continuous_observation.observation.rel_v_y / self.v_max, -1, 1
            )
            continuous_acceleration = np.clip(
                current_continuous_observation.observation.rel_a_y / self.a_max, -1, 1
            )
        else:
    ...
    def check(
        ...
        # TODO: The paper distinguishes between the latest curriculum step and the previous ones. In previous curriculum steps, the agent is rewarded for immediately reaching the goal state. For the latest curriculum step, however, the paper states that the episode should only be terminated in success if the agent “has been in that curriculum step’s discrete states for at least one second without interruption.”
    ...
    def continuous_action(self, action: int):
        ...
        # if self.direction == "x":
        #     print(
        #         f"Outgoing: {self.direction=}, {self.action_values.pitch=}, {action=}"
        #     )
        # else:
        #     print(f"Outgoing: {self.direction=}, {self.action_values.roll=}, {action=}")
        # If action == 2 (do nothing)
