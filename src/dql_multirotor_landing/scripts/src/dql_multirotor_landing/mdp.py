...
class AbstractMdp(ABC):
    _current_shaping_value = RewardShapingValue()
    _previous_shaping_value = RewardShapingValue()
    _info: Dict[str, Any] = {}
    _step_count = 0
    ...
    def _continuous_action_x(self, action_x: int):
    ...
    def _continuous_action_y(self, action_y: int):
    ...
    def _check_contact(self):
        return self._current_continuous_observation.contact
        # Section 3.3.6
        # Discussed briefly when explaining the rewards.

    def _check_flyzone_x(self):
        return (
    ...
    def _check_flyzone_z(self):
        return self._current_continuous_observation.abs_p_z > self._flyzone_z[1]

    def _check_minimum_altitude(self):
        return self._current_continuous_observation.abs_p_z < self._minimum_altitude

    def _check_step_count(self):
        return self._step_count >= (self._t_max * self._f_ag)
...
class TrainingMdp(AbstractMdp):
    ...
    _current_continuous_action = Action(pitch=0, roll=0, yaw=0, v_z=-0.1)
    _current_shaping_value = RewardShapingValue(0, 0, 0)
    _previous_shaping_value = RewardShapingValue(0, 0, 0)
    ...
    def discrete_state(
        ...
        if self._direction == "x":
            self._current_discrete_state = self._discrete_state_x()
        else:
            self._current_discrete_state = self._discrete_state_y()
    ...
    def reward(self) -> float:
        ...
        if self._direction == "x":
            ...
            continuous_position = self._current_continuous_observation.rel_p_y
            continuous_velocity = self._current_continuous_observation.rel_v_y
        ...
        if self._direction == "x":
            normalized_angle = self._current_continuous_action.pitch / self._theta_max
        else:
            normalized_angle = self._current_continuous_action.roll / self._theta_max
        ...
        self._previous_shaping_value = replace(self._current_shaping_value)
        self._current_shaping_value = RewardShapingValue(
            self._w_p * np.abs(normalized_continuous_position),
            self._w_v * np.abs(normalized_continuous_velocity),
            self._w_theta * np.abs(normalized_angle),
        )
        # Eq 24
        r_p_max = (
            np.abs(self._w_p)
        ...
        # Kinda equation 20
        r_p = np.clip(
            self._current_shaping_value.position
            - self._previous_shaping_value.position,
            -r_p_max,
            r_p_max,
        )
        r_v = np.clip(
            self._current_shaping_value.velocity
            - self._previous_shaping_value.velocity,
            -r_v_max,
            r_v_max,
        )
        r_theta = (
            self._w_theta
            * (
                np.abs(self._current_shaping_value.angle)
                - np.abs(self._previous_shaping_value.angle)
            )
            / self._theta_max
            * self._limits.velocity[current_working_curriculum_step]
        )
    ...
    def continuous_action(self, action: int):
        if self._direction == "x":
            self._continuous_action_x(action)
        else:
            self._continuous_action_y(action)
    ...
    def reset(self):
        ...
        self._current_shaping_value = RewardShapingValue(0, 0, 0)
        self._previous_shaping_value = RewardShapingValue(0, 0, 0)
...
class SimulationMdp(AbstractMdp):
    ...
    def discrete_state(
        ...
        return self._discrete_state_x(), self._discrete_state_y()
    ...
    def reset(self):
        ...
        self._current_continuous_action = Action(
            pitch=0.0, roll=0.0, yaw=np.pi / 4, v_z=-0.1
        )
