...
class TrainingLandingEnv(AbstractLandingEnv):
    ...
    def reset(self) -> Tuple[int, int, int, int, int]:
        ...
        if self._direction == "x":
            ...
            if self._working_curriculum_step == 0:
                ...
                mu = 0
                sigma = self._p_max / 3
                y_init = np.random.normal(mu, sigma)
            else:
                # Not much it is said, but successive initial states are choosen uniformly
                # within he fly zone
                y_init = np.random.uniform(self._flyzone_x[0], self._flyzone_x[1])
            x_init = 0.0
        ...
        initial_drone.pose.position.y = np.clip(
            ...
            moving_platform.pose.position.y + self._flyzone_y[0],
            moving_platform.pose.position.y + self._flyzone_y[1],
        )
    ...
    def step(self, action: int):  # type: ignore
        ...
        continuous_action = self._mdp.continuous_action(action)
